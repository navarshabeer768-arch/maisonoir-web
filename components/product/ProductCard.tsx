      ...
      <Link href={`/product/${product.slug}`}>
        ...
        <button onClick={e => { e.preventDefault(); setWishlisted(!wishlisted); toast.success(wishlisted ? 'Removed' : 'Saved') }}
          className={`absolute top-3 right-3 z-10 w-8 h-8 flex items-center justify-center border transition-all duration-300 ${
            wishlisted ? 'bg-[#C9A84C] border-[#C9A84C] text-white' : 'bg-white border-[rgba(42,36,32,0.15)] text-[#C9A84C] hover:bg-[#C9A84C] hover:text-white'
          }`}>
        ...
        <div className="relative aspect-[3/4] bg-[#F5F0E8] overflow-hidden">
          {primaryImage ? (
            <Image src={primaryImage.url} alt={primaryImage.alt_text ?? product.name} fill
              ...
              className="object-cover transition-transform duration-700 group-hover:scale-105" priority={priority} />
          ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: hovered ? 1 : 0 }}
            className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-[rgba(42,36,32,0.75)] to-transparent">
            <button onClick={e => { e.preventDefault(); if (defaultVariant) { addItem(product, defaultVariant); toast.success('Added to cart') } }}
              className="btn-gold w-full text-[8px] py-2.5">Add to Cart</button>
          </motion.div>
        </div>

        {/* Info */}
        <div className="p-4">
          <p className="text-[8px] tracking-[2.5px] uppercase text-[#C9A84C] mb-1 font-semibold">{(product as any).brand?.name}</p>
          <h3 className="font-display text-[17px] font-light mb-1 text-[#2A2420] leading-tight">{product.name}</h3>
          <p className="text-[9px] uppercase text-[#9A8A7A] mb-3 truncate">
            {(product as any).concentration?.toUpperCase()} · {(product as any).gender_target}
          </p>
          {(product.review_count ?? 0) > 0 && (
            <div className="flex items-center gap-1.5 mb-2">
              <div className="flex gap-0.5">
                {[1,2,3,4,5].map(s => (
                  <svg key={s} className={`w-2.5 h-2.5 ${s <= Math.round(product.average_rating ?? 0) ? 'text-[#C9A84C]' : 'text-[#D5C9BA]'}`} viewBox="0 0 20 20" fill="currentColor">
          ...
          <div className="flex items-baseline gap-2">
            <span className="font-display text-xl text-[#2A2420]">
              {defaultVariant ? qar(defaultVariant.price) : '—'}
            </span>
            {isOnSale && (
              <span className="text-sm text-[#9A8A7A] line-through">{qar(defaultVariant!.compare_at_price!)}</span>
            )}
          </div>
        </div>
      </Link>
