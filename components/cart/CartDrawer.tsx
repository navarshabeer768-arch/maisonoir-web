...
export function CartDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const shipping = subtotal >= freeShipping ? 0 : 55
            ...
            {subtotal < freeShipping ? (
              <div className="px-6 py-3 border-b border-[rgba(42,36,32,0.06)] bg-amber-50">
                <p className="text-[9px] tracking-[1px] text-[#6B5E4A] mb-1.5">
                  Add <span className="text-[#C9A84C] font-semibold">QAR {remaining.toFixed(0)}</span> more for free shipping
                </p>
              ...
              <div className="px-6 py-2.5 border-b border-emerald-100 bg-emerald-50">
                <p className="text-[9px] tracking-[1px] text-emerald-700 font-medium">✓ You qualify for free shipping</p>
              </div>
            )}
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-4 px-7">
                  ...
                  <button onClick={onClose}>
                    <Link href="/shop" className="btn-dark mt-2 text-[9px] px-6 py-3">Browse Fragrances</Link>
                  </button>
                </div>
              ) : (
                    ...
                    return (
                      <motion.div key={item.variant.id} layout exit={{ opacity: 0, x: 20 }} className="flex gap-4 p-5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[8px] tracking-[2px] uppercase text-[#C9A84C] font-semibold mb-0.5">{(item.product as any).brand?.name}</p>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-display text-lg text-[#2A2420]">{qar(item.variant.price * item.quantity)}</span>
                          </div>
                        </div>
              ...
              <div className="p-6 border-t border-[rgba(42,36,32,0.08)] bg-[#FAF7F2]">
                <div className="space-y-2 mb-5">
                  <div className="flex justify-between text-[12px] text-[#6B5E4A]">
                    <span>Subtotal</span><span className="text-[#2A2420]">{qar(subtotal)}</span>
                  </div>
                  <div className="flex justify-between text-[12px]">
                    <span className="text-[#6B5E4A]">Shipping</span>
                    <span className={shipping === 0 ? 'text-emerald-600 font-medium' : 'text-[#2A2420]'}>{shipping === 0 ? 'Free' : qar(shipping)}</span>
                  </div>
