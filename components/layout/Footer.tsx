    ...
    <footer className="bg-[#2A2420] text-[#F0EAD6] pt-20 pb-10 px-6 md:px-12 mt-20">
      <div className="max-w-[1400px] mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-16">
          ...
          {/* Collection */}
          <div>
            <p className="text-[9px] tracking-[4px] uppercase text-[#C9A84C] mb-5 font-semibold">Collection</p>
            <ul className="space-y-3">
              {[
                { href: '/shop', l: 'All Fragrances' },
                { href: '/shop?filter=new', l: 'New Arrivals' },
                { href: '/shop?filter=bestseller', l: 'Best Sellers' },
                { href: '/shop?category=arabic-perfumes', l: 'Arabic Collection' },
                { href: '/shop?category=luxury-gift-sets', l: 'Gift Sets' },
                { href: '/discovery', l: 'Scent Discovery' },
              ].map(({ href, l }) => (
                <li key={href}>
          ...
          {/* Account */}
          <div>
            <p className="text-[9px] tracking-[4px] uppercase text-[#C9A84C] mb-5 font-semibold">Account</p>
            <ul className="space-y-3">
              {[
                { href: '/account', l: 'My Account' },
                { href: '/account/orders', l: 'My Orders' },
                { href: '/account/wishlist', l: 'Wishlist' },
                { href: '/account/loyalty', l: 'Loyalty Rewards' },
                { href: '/auth/login', l: 'Sign In' },
                { href: '/auth/register', l: 'Create Account' },
              ].map(({ href, l }) => (
                <li key={href}>
                  <Link href={href} className="text-[11px] text-[#8A7A6A] hover:text-[#C9A84C] transition-colors">{l}</Link>
                </li>
          ...
          {/* Help */}
          <div>
            <p className="text-[9px] tracking-[4px] uppercase text-[#C9A84C] mb-5 font-semibold">Help</p>
            <ul className="space-y-3">
              {[
                { href: '/support', l: 'Customer Care' },
                { href: '/faq', l: 'FAQ' },
                { href: '/returns', l: 'Returns Policy' },
                { href: '/shipping', l: 'Shipping Info' },
                { href: '/terms', l: 'Terms of Service' },
                { href: '/privacy', l: 'Privacy Policy' },
              ].map(({ href, l }) => (
                <li key={href}>
                  <Link href={href} className="text-[11px] text-[#8A7A6A] hover:text-[#C9A84C] transition-colors">{l}</Link>
                </li>
        ...
        <div className="border-t border-[rgba(201,168,76,0.1)] pt-8 flex flex-wrap justify-between items-center gap-4">
          ...
          <p className="text-[9px] text-[#5A5048]">Doha, Qatar · hello@maisonoir.com</p>
        </div>
      </div>
    </footer>
  )
