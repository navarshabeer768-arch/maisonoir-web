...
async function getProduct(slug: string) {
  try {
    ...
    const { data: product } = await supabase
      ...
      `)
      .eq('slug', slug)
      .neq('status', 'archived')  // show active AND draft products
      .single()
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  // Fetch related and reviews safely
  let reviews: any[] = []
  let related: any[] = []
  ...
  const sortedImages = [...((product.images as any[]) ?? [])].sort((a, b) => a.display_order - b.display_order)
  ...
  return (
    <div className="bg-[#FAF7F2]">
      ...
      {/* Breadcrumb */}
      <div className="px-6 md:px-12 py-4 border-b border-[rgba(42,36,32,0.06)]">
