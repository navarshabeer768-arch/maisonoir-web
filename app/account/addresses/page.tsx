    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL) {
      return (
        <AccountLayout profile={{ email: '', first_name: 'Guest', last_name: '', role: 'customer' }}>
          <div>
            <h1 className="font-display text-4xl font-light text-[#2A2420] mb-8 capitalize">Addresses</h1>
    ...
    return (
      <AccountLayout profile={safeProfile as any}>
        <div>
          <h1 className="font-display text-4xl font-light text-[#2A2420] mb-8 capitalize">Addresses</h1>
          <div className="bg-white border border-[rgba(42,36,32,0.07)] p-10 text-center shadow-sm">
            <p className="text-[#9A8A7A] text-sm">Your Addresses will appear here.</p>
          </div>
        </div>
      </AccountLayout>
    )
  } catch (e: any) {
    console.error('Addresses page error:', e)
    redirect('/auth/login')
  }
}
